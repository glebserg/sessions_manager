...
from services.background.access_locker import AccessLocker
...
async def access_locker():
    """Возвращает блокировщика доступа."""
    task = AccessLocker(
...
async def counter_updater():
    """Возвращает объект для обновления счётчиков."""
    task = CounterUpdater(
        proc_collector=ProcessesCollector(),
        session_collector=SessionsCollector(),
        limit_service=get_limit_service(),
