...
def create_system_app() -> None:
    ...
    app_exist: AppModel | None = repo_apps.get_first_or_none_by_filters(name=NAME_APP_SYSTEM)
    if not app_exist:
...
async def create_local_users_to_db() -> None:
    ...
    for local_user in local_users:
        user_exist: UserModel | None = repo_users.get_first_or_none_by_filters(username=local_user.username)
        if not user_exist:
