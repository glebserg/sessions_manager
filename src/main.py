...
from dependencies.init_app import create_local_users_to_db, create_system_app
from dependencies.user import get_user_service
from routers import apps, users
...
from services.background import access_locker, counter_updater
from services.user import UserService
from shemes.user import UserList
...
async def lifespan(_: FastAPI):
    ...
    scheduler.add_job(
        counter_updater,
        "interval",
        seconds=UPDATE_COUNTER_RATE,
        next_run_time=datetime.now(),
    )
    scheduler.add_job(
        access_locker,
        "interval",
        seconds=USER_INSPECT_RATE,
        next_run_time=datetime.now(),
    )
    scheduler.start()
    yield
